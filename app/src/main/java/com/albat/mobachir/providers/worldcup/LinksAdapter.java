...
public class LinksAdapter extends InfiniteRecyclerViewAdapter {
    ...
    public LinksAdapter(List<LinkItem> data, Context context, OnOverViewClick click) {
        super(context, null);
        this.data = data;
    ...
    protected void doBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        LinkItem link = data.get(position);
        MatchViewHolder matchViewHolder = (MatchViewHolder) holder;
        matchViewHolder.channelName.setText(link.getLinkName());
    }
    ...
    public interface OnOverViewClick {
        void onOverViewSelected(LinkItem link);
    }

    public void addAll(List<LinkItem> items) {
        data.addAll(items);
    }
